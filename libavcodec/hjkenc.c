...
#define FEQ(m, n) (fabs(m - n) < 1e-9)
#define FNEQ(m, n) (fabs(m - n) >= 1e-9)
...
static int hjkenc_print_error(void *log_ctx, HJKENCSTATUS err,
                             const char *error_string)
{
    const char *desc = NULL;
    int ret = 0;
    ret = hjkenc_map_error(err, &desc);
...
static int hjkenc_check_capabilities(AVCodecContext *avctx)
{
    ...
    if (ctx->b_ref_mode == HJK_ENC_BFRAME_REF_MODE_EACH && ret != 1) {
...
static av_cold int hjkenc_setup_device(AVCodecContext *avctx)
{
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_HJK || avctx->pix_fmt == AV_PIX_FMT_D3D11 || avctx->hw_frames_ctx || avctx->hw_device_ctx) {
        ...
        if (!dl_fn->hjkenc_device_count) {
            av_log(avctx, AV_LOG_FATAL, "No HJKENC capable devices found\n");
            return AVERROR_EXTERNAL;
        }
...
static av_cold void set_constqp(AVCodecContext *avctx)
{
    ...
    if (ctx->init_qp_p >= 0) {
        ...
        } else if (FNEQ(avctx->i_quant_factor, 0.0) && FNEQ(avctx->b_quant_factor, 0.0)) {
...
static av_cold void set_vbr(AVCodecContext *avctx)
{
    ...
    if (ctx->init_qp_i < 0) {
        if (FNEQ(avctx->i_quant_factor , 0.0) && FNEQ(avctx->b_quant_factor , 0.0)) {
...
static av_cold int hjkenc_setup_mjpeg_config(AVCodecContext *avctx)
{
    ...
    if (avctx->refs >= 0) {
        ...
        mjpeg->maxNumRefFrames = avctx->refs;
    }
    if (avctx->gop_size >= 0) {
...
static HJK_ENC_BUFFER_FORMAT hjkenc_map_buffer_format(enum AVPixelFormat pix_fmt)
{
    ...
    case AV_PIX_FMT_NV12:
        return HJK_ENC_BUFFER_FORMAT_NV12_PL;
                                            ...
                                            HJK_ENC_SEI_PAYLOAD *sei_data)
{
    ...
    case AV_CODEC_ID_MJPEG:
        ...
        if (sei_data) {
            ...
            params->codecPicParams.mjpegPicParams.seiPayloadArrayCnt = 1;
        }
...
{
    HJKENCSTATUS hjk_status = 0;
    HjkencSurface *tmp_out_surf = NULL, *in_surf = NULL;
    int res = 0, res2 = 0;
    HJK_ENC_SEI_PAYLOAD *sei_data = NULL;
    size_t sei_size = 0;
    ...
    if (frame) {
        ...
        if (ctx->a53_cc && av_frame_get_side_data(frame, AV_FRAME_DATA_A53_CC)) {
            if (ff_alloc_a53_sei(frame, sizeof(HJK_ENC_SEI_PAYLOAD), (void**)&sei_data, &sei_size) < 0) {
        ...
        hjkenc_codec_specific_pic_params(avctx, &pic_params, sei_data);
    } else {
...
{
    HjkencSurface *tmp_out_surf = NULL;
    int res = 0, res2 = 0;
                          ...
                          const AVFrame *frame, int *got_packet)
{
    ...
    if (!ctx->encoder_flushing) {
        res = ff_hjkenc_send_frame(avctx, frame);
        if (res < 0)
            return res;
    }

    res = ff_hjkenc_receive_packet(avctx, pkt);
